class NcCharterSchools::Scraper
 ...
 # Scraping data on charter schools managed by North Carolina Department of Public Instruction
  ...
  def self.scrape_name
    @@name ||= data_load.map do |data|
      data.children.text.lstrip.scan(/[a-zA-Z\W\s]+(?=Charter Code)/)
    end.flatten
  end
  ...
  def self.scrape_city_state
    @@city_state ||= data_load.map do |data|
      data.children.text.scan(/(?<=\n)[a-zA-Z\s\W]+\s\NC\s\d+/)
    end.flatten
  end
  ...
  def self.scrape_telephone
    @@telephone ||= data_load.map {|data| data.children.text.scan(/Phone:\s\d+\.\d+\d.\d+/).join.gsub("Phone: ", "")}
  end
  ...
  def self.scrape_grade
    @@grade ||= doc.css("tr.highlight-row").map {|data| data.children[3].text}
  end
  ...
  def self.scrape_nc_county
    @@nc_counties ||= county_doc.css("div.map-select option").select do |data|
      data.attributes.size == 3
    end.map { |data| data.children.text }
  end
end